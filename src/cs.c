...
csc* csc_spalloc(c_int m, c_int n, c_int nzmax, c_int values, c_int triplet) {
  ...
  if (!A) return QPALM_NULL;            /* out of memory */
  ...
  A->x     = values ? csc_malloc(nzmax,  sizeof(c_float)) : QPALM_NULL;
...
csc* triplet_to_csc(const csc *T, c_int *TtoC) {
  ...
  Tx = T->x;
  nz = T->nz;
  C  = csc_spalloc(m, n, nz, Tx != QPALM_NULL, 0);     /* allocate result */
  w  = csc_calloc(n, sizeof(c_int));                  /* get workspace */

  if (!C || !w) return csc_done(C, w, QPALM_NULL, 0);  /* out of memory */
  ...
  for (k = 0; k < nz; k++) {
    ...
    if (Cx) {
      ...
      if (TtoC != QPALM_NULL) TtoC[k] = p;  // Assign vector of indices
    }
  }
  return csc_done(C, w, QPALM_NULL, 1);     /* success; free w and return C */
}
...
csc* csc_to_triu(csc *M) {
  ...
  M_triu = triplet_to_csc(M_trip, QPALM_NULL);
