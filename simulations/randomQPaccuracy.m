...
% n_values = 3:4;
n_values = logspace(-9, -2, 20);
nb_n = length(n_values);

for i = 1:nb_n
    
    options.EPS_ABS = n_values(i);
...
loglog(n_values(1:length(Tqpalm_matlab)), Tqpalm_matlab, 'b',...
...
loglog(n_values(1:length(Tqpalm_matlab)), Iter_qpalm_matlab, 'b',...
    n_values(1:length(Tosqp)), Iter_osqp, 'r',...
    n_values(1:length(Tqpoases)), Iter_qpoases, 'g',...
    n_values(1:length(Tgurobi)), Iter_gurobi, 'k');
